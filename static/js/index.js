// Create a template
const template = Handlebars.compile(document.querySelector('#address-template').innerHTML);
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    socket.on('connect', () => {
        ...
        document.querySelector('#send-button').onclick = () => {
            ...
            const time = current_time.slice(0, 3) + ' ' + current_time.slice(-12);

            // Send
            socket.send({ 'name': name, 'message': message, 'time': time, 'room': room });
    ...
    socket.on('message', data => {
        ...
        const content = template({ 'name': data.name, 'message': data.message, 'time': data.time });
